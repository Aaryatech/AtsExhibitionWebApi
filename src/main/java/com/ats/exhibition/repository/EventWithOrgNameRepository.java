...
public interface EventWithOrgNameRepository extends JpaRepository<EventWithOrgName, Integer> {
	...
	@Query(value = "SELECT c.*, o.org_name FROM t_events c,m_organiser o WHERE c.event_id=:eventId AND o.org_id=c.org_id", nativeQuery = true)
	List<EventWithOrgName> getAllEventsByEventId(@Param("eventId") int eventId);

	@Query(value = "SELECT c.*, o.org_name FROM t_events c,m_organiser o WHERE c.is_used=1 AND o.org_id=c.org_id", nativeQuery = true)
