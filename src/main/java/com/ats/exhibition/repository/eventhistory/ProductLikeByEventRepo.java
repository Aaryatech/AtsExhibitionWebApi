...
public interface ProductLikeByEventRepo extends JpaRepository<ProductLikeByEvent, Integer> {
	...
	@Query(value="SELECT t_visitor_product.visitor_product_id, t_visitor_product.visitor_id,"
			+ "COUNT(t_visitor_product.like_status) AS like_status, m_visitor.visitor_name, m_visitor.visitor_email, "
			+ "m_visitor.visitor_mobile, m_location.location_name FROM m_visitor, t_visitor_product, m_location WHERE"
			+ " t_visitor_product.event_id=:eventId  AND t_visitor_product.product_id=:prodId AND t_visitor_product.visitor_id=m_visitor.visitor_id  "
			+ "AND m_location.location_id=m_visitor.location_id AND t_visitor_product.like_status=1 GROUP BY t_visitor_product.visitor_id "
			+ "ORDER BY t_visitor_product.visitor_product_id ",nativeQuery=true)
	List<ProductLikeByEvent> visitorLikesListByProductId(@Param("eventId")int eventId,@Param("prodId") int prodId);
