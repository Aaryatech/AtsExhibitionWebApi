...
public interface GetScheduleHeaderRepository extends JpaRepository<GetSchedule, Integer>{
	...
	@Query(value="select s.schedule_id,s.event_id,e.event_name,s.date,s.day_name,s.is_used from m_exh_schedule s,m_events e where e.event_id=s.event_id and s.is_used=1 and s.event_id In(select event_id from m_events where org_id=:orgId)",nativeQuery=true)
	List<GetSchedule> findByOrgId(@Param("orgId")int orgId);
