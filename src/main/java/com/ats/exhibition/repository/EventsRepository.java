...
public interface EventsRepository extends JpaRepository<Events, Integer> {
	...
	@Query(value = "\r\n"
			+ "select p.* from t_visitor_event ve,m_events p where ve.visitor_id=:visitorId  AND ve.event_id=p.event_id AND ve.subscribe_status=1"
			+ "", nativeQuery = true)
	List<Events> getAllEventsWithVisitorId(@Param("visitorId") int visitorId);
