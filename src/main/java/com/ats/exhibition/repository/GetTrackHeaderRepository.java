...
public interface GetTrackHeaderRepository extends JpaRepository<GetTrackHeader, Integer> {
	...
	List<GetTrackHeader> getTotalKmOfEmpByExhiId(@Param("exhiId") int exhiId, @Param("fromDate") String fromDate,
	...
	List<GetTrackHeader> trakingHistoryBetweenDate(@Param("exhiId") int exhiId, @Param("fromDate") String fromDate,
			@Param("toDate") String toDate);

	@Query(value = "select th.*, e.emp_name from t_emp_tracking_header th, m_exh_employee e where th.is_used=1 and th.exhi_id =:exhiId "
			+ "and th.emp_id =:empId and th.emp_id = e.emp_id and th.date between :fromDate and :toDate", nativeQuery = true)
	List<GetTrackHeader> trakingHistoryBetweenDate(@Param("empId") int empId, @Param("exhiId") int exhiId,
	...
	@Query(value = "select th.*,e.emp_name from t_emp_tracking_header th,m_exh_employee e where th.is_used=1 "
			+ "and th.emp_id = e.emp_id and th.emp_id=:empId and th.date between :fromDate AND :toDate", nativeQuery = true)
	GetTrackHeader getTrackHeaderBetDateAndByEmpId(@Param("empId") int empId, @Param("fromDate") String fromDate,
			@Param("toDate") String toDate);
