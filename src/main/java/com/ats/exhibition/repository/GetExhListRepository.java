...
public interface GetExhListRepository extends JpaRepository<GetExhibitorsList, Integer> {
	...
	@Query(value = " SELECT  m_exhibitor.exh_id,m_exhibitor.exh_name, m_events.event_name, m_events.event_id,m_exhibitor.logo,"
			+ "m_exhibitor.exh_company, m_exhibitor.address,  m_exhibitor.company_type,  coalesce((Select "
			+ "t_visitor_exhibitor.like_status   From t_visitor_exhibitor   where  m_exhibitor.exh_id=t_visitor_exhibitor.exhibitor_id  "
			+ " AND t_visitor_exhibitor.visitor_id=:visitorId), 0) AS like_status  FROM  m_exhibitor, m_events, `t_event_exh_mapping` "
	...
	@Query(value = "SELECT m_exhibitor.exh_id,m_exhibitor.exh_name,m_events.event_name, m_events.event_id,m_exhibitor.logo,"
			+ "m_exhibitor.exh_company, m_exhibitor.address,  m_exhibitor.company_type,coalesce"
